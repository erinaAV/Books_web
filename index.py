...
def add_car():
    ...
    return render_template("add_car.html", title='Добавление книги', form=form)
...
def dealer(dealer_id):
    ...
    return render_template('dealer_info.html',
                           username=session['username'],
                           title='Просмотр информации об авторе',
                           dealer=dealer)
...
def add_dealer():
    ...
    if session['username'] == 'admin':
        ...
        if form.validate_on_submit():
            ...
            # редирект на главную страницу
            return redirect(url_for('index'))
        return render_template("add_dealer.html", title='Добавление автора', form=form)
#http://127.0.0.1:8080/
